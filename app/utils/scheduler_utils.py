...
async def load_and_send(source, app_config):
    loader_factory = LoaderFactory(app_config)
    loader = loader_factory.get_loader(source)
    await loader.load()
    if loader.data:
        sender_factory = SenderFactory(app_config)
...
async def add_all_jobs(app_config):
    async with AsyncSessionLocal() as session:  
        sources = await get_source_list(session, is_active=True)
        for source in sources:
            await add_job(source, app_config)
